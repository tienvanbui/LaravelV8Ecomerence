        ...
        <form action="{{ route('coupon.update',['coupon'=>$coupon]) }}" method="post">
          ...
          <div class="form-group">
            ...
            <select name="coupon_codition" id="coupon_codition" class="form-control">
                ...
                <option {{$coupon->coupon_codition == 0 ? 'selected' : ''}} value="0">discount as a money</option>
                <option {{$coupon->coupon_codition == 1 ? 'selected' : ''}}  value="1">discount as a percentage</option>
            </select>
          </div>
           <div class="form-group">
            <label for="coupon_price_discount">Discount Price:</label>
            <input type="number" class="form-control" id="coupon_price_discount" aria-describedby="coupon_price_discount" name="coupon_price_discount" value="{{$coupon->coupon_price_discount}}">
          </div>
          <div class="form-group">
            <label for="coupon_use_number" class="text-capitalize">number of coupons available:</label>
            <input type="number" class="form-control" id="coupon_use_number" aria-describedby="coupon_use_number" name="coupon_use_number" value="{{$coupon->coupon_use_number}}">
          </div>
